...
inf_related = [i for i, (combo, _) in enumerate(blocks) if 39 in combo and 0 in combo]
...
for n, triple in enumerate(orbit_representatives(), start=1):
    ...
    model.Add(X[0][first_index] == 1)
    model.Add(X[0][second_index] == 1)
    for i in inf_related:
        if i < min(first_index, second_index):
            model.Add(X[0][i] == 0)
