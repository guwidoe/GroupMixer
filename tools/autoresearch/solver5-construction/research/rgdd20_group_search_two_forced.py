...
forced = [tuple(sorted(int(x) for x in arg.split(','))) for arg in sys.argv[1:3]]
...
print('forced', forced, [[Gs[x] for x in combo] for combo in forced], 'status', solver.StatusName(res), 'wall', round(solver.WallTime(), 3))
