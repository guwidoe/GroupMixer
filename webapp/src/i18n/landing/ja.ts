...
const ADVANCED_SECTION = {
  ...
  buttonLabel: '高度なエディターを開く',
  supportingText:
    '高度なエディターでは、セッション、制約、ソルバー設定、詳細な結果分析まで細かく操作できます。',
};
...
const FAQS = {
  ...
  constraints: {
    ...
    answer:
      'はい。詳細オプションで「同じグループにする」「一緒にしない」「複数セッション」「属性バランス」などを設定できます。より高度な制御は高度なエディターで行えます。',
  },
  multiSession: {
  ...
  workspace: {
    question: '高度なエディターとは何ですか？',
    answer:
      '高度なエディターでは、セッション、制約、ソルバー設定、過去結果の再利用、詳細分析まで同じ強力なエンジンで扱えます。',
  },
};
