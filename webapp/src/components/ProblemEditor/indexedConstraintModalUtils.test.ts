import { describe, expect, it } from 'vitest';
import type { Constraint } from '../../types';
import { createSampleProblem } from '../../test/fixtures';
import { getIndexedConstraint, saveIndexedConstraint } from './indexedConstraintModalUtils';

describe('indexedConstraintModalUtils', () => {
  it('returns the indexed constraint when present', () => {
    const constraint: Constraint = {
      type: 'RepeatEncounter',
      max_allowed_encounters: 1,
      penalty_function: 'linear',
      penalty_weight: 2,
    };
    const problem = createSampleProblem({ constraints: [constraint] });

    expect(getIndexedConstraint(problem, 0)).toEqual(constraint);
    expect(getIndexedConstraint(problem, null)).toBeNull();
  });

  it('appends or replaces constraints without mutating the original problem', () => {
    const original: Constraint = {
      type: 'RepeatEncounter',
      max_allowed_encounters: 1,
      penalty_function: 'linear',
      penalty_weight: 2,
    };
    const replacement: Constraint = {
      type: 'ShouldStayTogether',
      people: ['p1', 'p2'],
      penalty_weight: 5,
    };
    const problem = createSampleProblem({ constraints: [original] });

    const replaced = saveIndexedConstraint(problem, replacement, 0);
    const appended = saveIndexedConstraint(problem, replacement, null);

    expect(replaced.constraints).toEqual([replacement]);
    expect(appended.constraints).toEqual([original, replacement]);
    expect(problem.constraints).toEqual([original]);
  });
});
