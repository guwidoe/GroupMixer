...
import { ManualEditorTopBar } from './ManualEditor/ManualEditorTopBar';
import { ManualEditorSidebar } from './ManualEditor/ManualEditorSidebar';
import { ManualEditorGroupColumn } from './ManualEditor/ManualEditorGroupColumn';
import { ManualEditorStoragePanel } from './ManualEditor/ManualEditorStoragePanel';
import { ManualEditorStatusBar } from './ManualEditor/ManualEditorStatusBar';
import { ManualEditorNotReady } from './ManualEditor/ManualEditorNotReady';
import { ManualEditorLeaveConfirmModal } from './ManualEditor/ManualEditorLeaveConfirmModal';
import type { Mode, PendingMove } from './ManualEditor/types';
import { cloneAssignments, groupBySessionAndGroup, snapshotToProblem } from './ManualEditor/utils';
import { useManualEditorDraft } from './ManualEditor/hooks/useManualEditorDraft';
import { useManualEditorEvaluation } from './ManualEditor/hooks/useManualEditorEvaluation';
import { useManualEditorNavigationGuard } from './ManualEditor/hooks/useManualEditorNavigationGuard';
import { pullNewConstraints, pullNewPeople } from './ManualEditor/pullHandlers';
import { canDrop } from './ManualEditor/moveUtils';
import { buildManualDraftSolution } from './ManualEditor/draftSolution';
import { discardAndContinue, saveAndContinue } from './ManualEditor/leaveActions';
...
function ManualEditor() {
  ...
  const handleDropPerson = async (personId: string, targetGroupId: string, sessionId: number) => {
    ...
    const fromGroupId = Object.entries(draftSchedule[sessionId] || {}).find(([_, list]) => list.includes(personId))?.[0];
    let beforeScore = {
      final_score: 0,
      unique_contacts: 0,
