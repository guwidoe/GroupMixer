        ...
        >
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1 min-w-0">
              {editingId === scenario.id ? (
                ...
                <div className="flex items-center space-x-2" onClick={() => onLoadScenario(scenario.id)}>
                  <h3 className="font-medium truncate" style={{ color: 'var(--text-primary)' }}>
                    {scenario.name}
                  </h3>
                  {scenario.isTemplate && <Star className="h-4 w-4 text-yellow-500 flex-shrink-0" />}
                </div>
              )}
            </div>
          </div>
          ...
          >
            <div className="flex items-center space-x-1">
              ...
              <span>{scenario.peopleCount} people</span>
            </div>
            <div className="flex items-center space-x-1">
              <Layers className="h-3 w-3" />
              <span>{scenario.groupsCount} groups</span>
            </div>
            <div className="flex items-center space-x-1">
              <Calendar className="h-3 w-3" />
              <span>{scenario.sessionsCount} sessions</span>
            </div>
            <div className="flex items-center space-x-1">
              <BarChart3 className="h-3 w-3" />
              <span>{scenario.resultsCount} results</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-1">
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onRenameStart(scenario.id, scenario.name);
                }}
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onDuplicate(scenario.id, scenario.name);
                }}
                className="p-1 transition-colors"
                style={{ color: 'var(--text-tertiary)' }}
                onMouseEnter={(e) => (e.currentTarget.style.color = 'var(--text-primary)')}
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onToggleTemplate(scenario.id);
                }}
                className="p-1 transition-colors"
                style={{ color: scenario.isTemplate ? 'var(--color-warning-500)' : 'var(--text-tertiary)' }}
                onMouseEnter={(e) => (e.currentTarget.style.color = 'var(--text-primary)')}
                onMouseLeave={(e) =>
                  (e.currentTarget.style.color = scenario.isTemplate ? 'var(--color-warning-500)' : 'var(--text-tertiary)')
                }
                title={scenario.isTemplate ? 'Remove from templates' : 'Add to templates'}
              >
