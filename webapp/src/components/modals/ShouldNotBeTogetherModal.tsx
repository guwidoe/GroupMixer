...
export function ShouldNotBeTogetherModal({ sessionsCount, initial, onCancel, onSave }: Props) {
  ...
  const getInitialState = () => {
    ...
    return {
      ...
      sessionScope: editing && initial?.type === 'ShouldNotBeTogether'
    ...
    <div className="fixed inset-0 modal-backdrop flex items-center justify-center z-50 p-4">
      <div className="rounded-lg p-4 sm:p-6 w-full max-w-2xl mx-auto modal-content max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>{editing ? getConstraintEditLabel('ShouldNotBeTogether') : getConstraintAddLabel('ShouldNotBeTogether')}</h3>
          <button 
            onClick={onCancel} 
