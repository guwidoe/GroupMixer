...
async fn schema_ids_and_error_codes_match_across_surfaces() {
    ...
    let server_schema_ids: HashSet<_> =
    ...
    let server_error_codes: HashSet<_> =
...
async fn related_help_targets_resolve_consistently_across_surfaces() {
    ...
    for operation_id in bootstrap_spec().top_level_operation_ids {
        ...
        let server_related: HashSet<_> = server_help
            ...
            .collect();
        assert_eq!(
            server_related, contract_related,
            "HTTP related-help drift for {operation_id}"
        );
        for related in &server_help.related_operations {
            assert_eq!(
                related.help_path,
                format!("/api/v1/help/{}", related.operation_id)
            );
        }

        let wasm_help =
            build_operation_help_response(operation_id).expect("WASM local help should resolve");
        let wasm_related: HashSet<_> = wasm_help
            ...
            .collect();
        assert_eq!(
            wasm_related, contract_related,
            "WASM related-help drift for {operation_id}"
        );
