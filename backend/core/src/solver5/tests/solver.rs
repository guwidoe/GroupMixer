...
#[test]
fn solver5_solves_catalog_backed_12_3_17_case() {
    let input = pure_input(12, 3, 17);
    let solver = SearchEngine::new(&input.solver);
    let result = solver
        .solve(&input)
        .expect("catalog-backed nkts construction should solve 12-3-17");
...
#[test]
fn solver5_solves_catalog_backed_17_3_25_case() {
    let input = pure_input(17, 3, 25);
    let solver = SearchEngine::new(&input.solver);
    let result = solver
        .solve(&input)
        .expect("catalog-backed kts construction should solve 17-3-25");
