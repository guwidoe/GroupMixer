...
- The current kept search baseline is guarded record-to-record acceptance with a linear `2 -> 0` uphill band over `max(iteration_fraction, time_fraction)`. That is the score to beat.
- Naive restart, naive best-of-k neighborhood enrichment, exact-undo tabu, pure LAHC, naive equal-slice multi-start, naive whole-session ALNS rebuild, and naive constructor-seeded crossover have all been tried and retired.
- Stronger elite-donor memetic follow-ups have now also been tried: build a small diverse donor set from alternate constructor seeds, briefly polish donors with record-to-record, then try compatibility-aware one-/two-session splices. A relaxed version that allows good offspring to re-enter the current trajectory improved the memetic family further (best bounded memetic follow-up `99.17`) but still lost to the `99.016` kept champion.
- A first **persistent elite-population** memetic run has now also been tried. It materially changed the architecture by keeping polished donors across the run and reinserting offspring into the pool. That first implementation regressed to `99.69`: fixed-iteration got better, but fixed-time `large_gender` drift dominated. If the memetic family is revisited later, the only justified next step is a more selective trigger / compatibility gate.
- A first **freedom-aware GRASP constructor** run has now also been tried as a full replacement for the baseline construction path before the kept record-to-record improver. It used pair-repeat, forbidden-pair, pair-meeting, attribute-balance, should-together, and future-freedom slack signals in a restricted-candidate-list assignment process. The family was directionally alive on social_golfer, large_gender, transfer-balance, and synthetic, but it regressed badly overall to `101.77` because `stretch_sailing_trip_demo_real` worsened to `1.1387`. If GRASP is revisited later, do **not** replace the global constructor wholesale again; instead try it as targeted diversification, restart seeding, or a selective portfolio input.
- A first **targeted ALNS / LNS restricted repair** run has now also been tried. On long stagnation it picked hotspot session/group pairs using repeated-contact pressure, locked immovables and active clique members, enumerated a capped exact reassignment over the remaining movable people across the two groups, and only accepted strictly improving repairs before resuming record-to-record search. This was the strongest fresh post-memetic direction so far, reaching `99.1311`: sailing and social_golfer improved meaningfully while runtime stayed stable, but synthetic partial-attendance regressed to `1.0187` and the run still missed the `99.016` champion.
- A second **targeted ALNS / LNS** follow-up has now also been tried, augmenting hotspot ranking with immediate local move potential (best swap/transfer preview delta inside the candidate pair) instead of using repeat pressure alone. That nudged the family slightly better again to `99.0947`: sailing stayed strong and synthetic improved from the first ALNS follow-up, but the gap to the `99.016` champion remained.
- A third **targeted ALNS / LNS** follow-up then tried chaining one overlapping second-stage repair in the same session to widen the local neighborhood without paying for a full three-group exact kernel. That removed almost all of the synthetic miss (`1.0011`) but gave back transfer/social_golfer quality and landed at `99.1123`, slightly worse than the second follow-up. Conclusion: this restricted-repair ALNS family is alive but likely near saturation without a richer region model. The best next **fresh** direction is now stage-aware macro-operator selection or a structural-signature region selector.
